...
- **Cap signals are evidence-checked too, not just dimensions.** A signal only fires a scoring cap when its citation validates against the transcript, same bar as per-dimension evidence - closes a real gap an external review of this repo flagged before submission.
